...
bool TypeModel::ParseFromDescriptor(const pb::Descriptor *type) {
  ...
  if (type->full_name() == google::protobuf::Empty::descriptor()->full_name()) {
    ...
    cpp_name_ = type->field(0)->enum_type()->name();
    csharp_name_ = type->field(0)->enum_type()->name();
    wrapper_name_ = name_ + "_wrapper__";
  } else {
    name_ = type->name();
    cpp_name_ = type->name();
    csharp_name_ = type->name();
...
bool TypeModel::ParseFromFieldDescriptor(const pb::FieldDescriptor *field) {
  ...
  switch (field->type()) {
    ...
    case pb::FieldDescriptor::TYPE_ENUM:
      ...
      cpp_name_ = field->enum_type()->name();
      csharp_name_ = field->enum_type()->name();
...
ModuleModel::ModuleModel(const google::protobuf::FileDescriptor *file) {
  CreateCodeModel(file, &services_);
}

bool CreateModuleModel(const google::protobuf::FileDescriptor *file,
                       ModuleModel *module_model) {
  // *module_model = new ModuleModel{ file };
  return true;
}
