...
std::string GetStubDeclarations(const pb::FileDescriptor *file) {
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      ...
      printer.Indent();
      printer.Print(vars, "using ServiceInterfaceType = $service_name$;\n\n");
      printer.Print(vars, "$service_name$_Stub(const $service_name$_Stub &impl) = delete;\n");
      printer.Print(vars, "$service_name$_Stub &operator=(const $service_name$_Stub &impl) = delete;\n");
      printer.Print(vars, "explicit $service_name$_Stub($service_name$* impl)\n");
      ...
      printer.Print(vars, "bool CallMethod(nanorpc::ServerContextInterface *context, const nanorpc::RpcCall &rpc_call, nanorpc::RpcResult *rpc_result) override;\n\n");
      printer.Outdent();
...
std::string GetStubDefinitions(
  ...
  std::string output;
  {
    ...
    for (const auto &service : models) {
      ...
      printer.Print(vars, "bool $service_name$_Stub::CallMethod(nanorpc::ServerContextInterface *context, const nanorpc::RpcCall &rpc_call, nanorpc::RpcResult *rpc_result) {\n");
...
std::string GetStubFactoryGetter(const pb::FileDescriptor *file) {
  std::string output;
  {
    // Scope the output stream so it closes and finalizes output to the string.
    pb::io::StringOutputStream output_stream(&output);
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      const auto service = file->service(i);
      vars["service_name"] = FullyQualifiedCppName(service->full_name());

      /* clang-format off */
      //printer.Print(vars, "template<>\n");
      //printer.Print(vars, "inline std::unique_ptr<nanorpc::ServiceStubFactoryInterface> GetServiceStubFactory($service_name$ *impl) {\n");
      //printer.Indent();
      //printer.Print(vars, "return std::make_unique<$service_name$_StubFactory<$service_name$>>(impl);\n");
      //printer.Outdent();
      //printer.Print(vars, "}\n\n");

      //printer.Print(vars, "template<>\n");
      //printer.Print(vars, "inline std::unique_ptr<nanorpc::ServiceStubFactoryInterface> GetServiceStubFactory(std::shared_ptr<$service_name$> impl) {\n");
      //printer.Indent();
      //printer.Print(vars, "return std::make_unique<$service_name$_StubFactory<$service_name$>>(impl);\n");
      //printer.Outdent();
      //printer.Print(vars, "}\n\n");
